...
struct ThinkingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 6) {
                Circle().fill(Color.cyan.opacity(0.8)).frame(width: 10, height: 10).scaleEffect(dotCount >= 1 ? 1.2 : 0.8)
                Circle().fill(Color.mint.opacity(0.8)).frame(width: 10, height: 10).scaleEffect(dotCount >= 2 ? 1.2 : 0.8)
                Circle().fill(Color.blue.opacity(0.8)).frame(width: 10, height: 10).scaleEffect(dotCount >= 3 ? 1.2 : 0.8)
                Text("Thinking…").font(.caption).foregroundColor(.white.opacity(0.7))
            }
            Text(streamedText.isEmpty ? "..." : streamedText)
                .font(.body).foregroundColor(.primary).padding(12)
                .background(
                    ZStack {
                        RoundedRectangle(cornerRadius: 18).fill(.ultraThinMaterial)
                        RoundedRectangle(cornerRadius: 18)
                            .stroke(
                                LinearGradient(colors: [.cyan.opacity(0.6), .blue.opacity(0.2), .cyan.opacity(0.6)],
        ...
        .onReceive(timer) { _ in
            withAnimation(.spring(response: 0.3, dampingFraction: 0.5)) { dotCount = (dotCount + 1) % 4 }
        }
        .onReceive(shimmerTimer) { _ in
            withAnimation(.linear(duration: 0.02)) { shimmerPhase += 0.01; if shimmerPhase > 1 { shimmerPhase = 0 } }
        }
    }
}
